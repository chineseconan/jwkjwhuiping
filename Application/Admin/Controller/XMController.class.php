...
class XMController extends Controller
{
    ...
    public function saveXm(){
        ...
        $xm_code = $Model
            ->field("xm_code,xm_id")
            ->where("xm_code='".$param['xm_code']."' and xm_status!='删除' and xm_id!='".$param['xm_id']."'")
            ->find();
        if(!empty($xm_code)){
            ...
            try{
                foreach($import['data'] as $val){
                    if($val['B']!=NULL && $val['A']!=NULL  ){
                        $xm_code = $Model
                            ->field("xm_code,xm_id")
                            ->where("xm_code='".$val['A']."' and xm_status!='删除'")
                            ->getField("xm_code");
                        if($xm_code!=""){
                            $str .= $val['A'].",";
                        }else{
	    ...
	    public function listindex()
    {
        ...
        $path = './Public/'. C("xmfilepath")."/".$xm_code." ".$xmdata["xm_name"]."_".$xmdata["xm_createuser"];
