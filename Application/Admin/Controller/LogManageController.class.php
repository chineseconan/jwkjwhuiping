...
class LogManageController extends BaseController
{
    ...
    public function index()
    {
        ...
        if (in_array('1',$roleId) || in_array('2',$roleId)||in_array('3',$roleId)) {
            ...
            $this->addLog('', '用户访问日志', '', '访问日志查询', '失败');
        }
        $this->addLog('', '用户访问日志', '', '访问日志查询', '成功');
        $this->display();
    }
        ...
        if ($queryParam['isshow']) {
            if ($queryParam['index'] == 0) {
                if (in_array('3',$roleId)) {
                    //普通用户日志
                    //$this->addLog('', '用户访问日志', '', '访问普通日志,查询条件:'.$conditon, '成功');
                    $where['opl_user'][$count] = [['notlike', '%(sysadmin)'], ['notlike', '%(sysadmin2)'], ['notlike', '%(secadmin)'], ['notlike', '%(secadmin2)'], ['notlike', '%(auditadmin)'], ['notlike', '%(auditadmin2)']];
                    ...
                    //系统管理员日志
                    //$this->addLog('', '用户访问日志', '', '访问系统管理员日志,查询条件:'.$conditon, '成功');
...
//                    print_r($model->_sql());die;
                ...
                if (in_array('3',$roleId)) {
                    //审计管理员日志
                    //$this->addLog('', '用户访问日志', '', '访问审计管理员日志,查询条件:'.$conditon, '成功');
                    ...
                    //安全管理员日志
                    //$this->addLog('', '用户访问日志', '', '访问安全管理员日志,查询条件:'.$conditon, '成功');
            ...
            //$this->addLog('', '用户访问日志', '', '访问系统运行日志,查询条件:'.$conditon, '成功');
            $where['opl_user'][$count] = [['like', '%(sysadmin)'], ['like', '%(sysadmin2)'],'or'];
    ...
    public function export()
    {
        ...
        if ($queryParam['isshow']) {
            if ($queryParam['index'] == 0) {
                if (in_array('3',$roleId)) {
                    //普通用户日志
                    $this->addLog('','对象修改日志','','导出普通用户日志','成功');
                    //$this->addLog('', '用户访问日志', '', '访问普通日志,查询条件:'.$conditon, '成功');
                    $where['opl_user'][$count] = [['notlike', '%(sysadmin)'], ['notlike', '%(sysadmin2)'], ['notlike', '%(secadmin)'], ['notlike', '%(secadmin2)'], ['notlike', '%(auditadmin)'], ['notlike', '%(auditadmin2)']];
                    $where['opl_logtype'][] = ['neq', '系统运行日志'];
                    ...
                    //系统管理员日志
                    $this->addLog('','对象修改日志','','导出系统管理员日志','成功');
                    //$this->addLog('', '用户访问日志', '', '访问系统管理员日志,查询条件:'.$conditon, '成功');
                    $where['opl_user'][$count] = [['like', '%(sysadmin)'], ['like', '%(sysadmin2)'],'or'];
                    $where['opl_logtype'][] = ['neq', '系统运行日志'];
                    $data = $model->field('opl_user,opl_ip,opl_logtype,opl_firstcontent,opl_result,opl_time')
                ...
                if (in_array('3',$roleId)) {
                    //审计管理员日志
                    $this->addLog('','对象修改日志','','导出审计管理员日志','成功');
                    //$this->addLog('', '用户访问日志', '', '访问审计管理员日志,查询条件:'.$conditon, '成功');
                    $where['opl_user'][$count] = [['like', '%(auditadmin)'], ['like', '%(auditadmin2)'],'or'];
                    $where['opl_logtype'][] = ['neq', '系统运行日志'];
                    $data = $model->field('opl_user,opl_ip,opl_logtype,opl_firstcontent,opl_result,opl_time')
                    ...
                    //安全管理员日志
                    $this->addLog('','对象修改日志','','导出安全管理员日志','成功');
                    //$this->addLog('', '用户访问日志', '', '访问安全管理员日志,查询条件:'.$conditon, '成功');
                    $where['opl_user'][$count] = [['like', '%(secadmin)'], ['like', '%(secadmin2)'],'or'];
                    $where['opl_logtype'][] = ['neq', '系统运行日志'];
                    $data = $model->field('opl_user,opl_ip,opl_logtype,opl_firstcontent,opl_result,opl_time')
            ...
            $this->addLog('','对象修改日志','','导出系统运行日志','成功');
            //$this->addLog('', '用户访问日志', '', '访问系统运行日志,查询条件:'.$conditon, '成功');
            $where['opl_user'][$count] = [['like', '%(sysadmin)'], ['like', '%(sysadmin2)'],'or'];
            $where['opl_logtype'][] = ['eq', '系统运行日志'];
            $data = $model->field('opl_user,opl_ip,opl_logtype,opl_firstcontent,opl_result,opl_time')
